...
def SimilarityIndexes(expList, simList, indextype, indexname):
    ...
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    pd.DataFrame(expList).to_csv(outdir + '/explist-' + indextype + indexname + '.txt', mode='a', sep=' ')
...
if __name__ == "__main__":
    ...
    for i in range(0, len(Labels)):
        Trajectories_List_List_List.append(InputTrajectories(Folder_Name + "//" + Labels[i]))
